...
// this function return 0 in case all the elements are negative
// if interviewer asks to return the maximum element then just check if all elements is negative and return the maximum one
...
int maxSubarraySum(vector<int> arr) {
	...
	int minimum_pre_sum = 0;
	int ans = 0;
	for (int i = 0; i < arr.size(); i++) {
		ans = max(ans, arr[i] - minimum_pre_sum);
		minimum_pre_sum = min(minimum_pre_sum, arr[i]);
	}
