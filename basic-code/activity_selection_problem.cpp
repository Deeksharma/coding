...
bool cmp(pair<int, int> &a, pair<int, int> &b) { // use reference in case of comparators, this will reduce the memory consumtion
...
int countActivites(vector<pair<int, int> > a) {
	...
	for (auto x : a) cout << "(" << x.ff << "," << x.ss << ")" << " ";
	cout << endl;
	int ans = 1;
	int endtime = a[0].ss;
	for (int i = 1; i < a.size(); i++) {
		while (i < a.size() && a[i].ff < endtime) {
			i++;
		}
		if (i == a.size()) break;
		ans++;
