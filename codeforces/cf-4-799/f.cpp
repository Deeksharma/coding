...
void solve() {
	...
	vector<vector<int>> possible_last_digits;
	for(int i=0;i<=9;i++){
		for(int j=0;j<=9;j++){
			for(int k=0;k<=9;k++){
				if((i+j+k)%10==3){
					possible_last_digits.pb({i,j,k});
				}
			}
		}
	}
	...
	for(int i=0;i<possible_last_digits.sz;i++){
		int a=possible_last_digits[i][0];
		int b=possible_last_digits[i][1];
		int c=possible_last_digits[i][2];
