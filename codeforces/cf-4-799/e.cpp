...
void solve() {
	...
	// loopbug(pre_sum, 0, n+1);
	int ans = INT_MIN;
	for(int i=0;i<=n;i++){
		int sum_to_find=pre_sum[i]+sum;

		auto x = upper_bound(pre_sum.begin()+i+1, pre_sum.end(), sum_to_find);
		x--;
		
		// varbug(sum_to_find);
		// varbug(x-pre_sum.begin());
		if(*x==sum_to_find){

			ans = max(ans, x-pre_sum.begin()-i);
			// cout<<ans<<endl;
		}
