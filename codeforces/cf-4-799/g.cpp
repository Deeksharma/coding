...
void solve() {
	...
	vector<int> validated_pairs(n-1,0);
	for(int i=0;i<n-1;i++){
		if(v[i]>=v[i+1]*2){
			validated_pairs[i]=1;
		}
	...
	// loopbug(validated_pairs,0,n-1);
	for(int i=1;i<n-1;i++){
		validated_pairs[i]+=validated_pairs[i-1];
	}
	// loopbug(validated_pairs,0,n-1);
	...
	for(int i=0;i<n-k;i++){
		if(i==0){
			if(validated_pairs[k-1]>=1){
				ans--;
			}
		}else{
			if(validated_pairs[k-1+i]-validated_pairs[i-1]>=1){
				ans--;
			}
		}
	}
