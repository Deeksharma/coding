...
void solve() {
	...
	vector<int> max_till_now(n,0);
	max_till_now[0] = v[0];
	for(int i=1;i<n;i++){
		max_till_now[i] = max(max_till_now[i-1], v[i]);
	}

	// loopbug(pre_sum, 0, n);
	// loopbug(max_till_now, 0, n);

	for(int i=0;i<q;i++){
		int x;
		cin>>x;

		int y = upper_bound(max_till_now.begin(), max_till_now.end(), x) - max_till_now.begin();
