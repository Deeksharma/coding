...
void solve() {
	...
	// Infinite case, when the 
	if((v[n-1]-(n-1-d<0?0:v[n-1-d]))>=c){
		cout<<"Infinity"<<endl;
		return;
	}

	// d/(k+1) * prefix_sum[n-1]-prefix_sum[k] + prefix_sum[n-1] - prefix_sum[n - d%(k+1)-1];
	int l=0, r = INT_MAX;
	int ans = -1;
	while(l<=r){
		int mid = l+(r-l)/2;

		int day = d/(mid+1); // no of times
		int remaining_days = d%(mid+1);

		int sum = day* (v[n-1]-(n-1-mid-1<0?0:v[n-1-mid-1])) + v[n-1] - (n-1 - remaining_days<0?0:v[n-1 - remaining_days]);
