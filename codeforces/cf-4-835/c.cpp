...
void solve() {
	...
	vector<int> v_right_max(n,-1);
	vector<int> v_left_max(n,-1);

	for(int i=1;i<n;i++){
		v_left_max[i] = max(v[i-1], v_left_max[i-1]);
	}
	for(int i=n-2;i>=0;i--){
		v_right_max[i] = max(v[i+1], v_right_max[i+1]);
	}

	// loopbug(v_right_max, 0, n);
	// loopbug(v_left_max, 0, n);

	for(int i=0;i<n;i++){
		int x =max(v_left_max[i], v_right_max[i]);
		cout<<v[i]-x<<" ";
	}
	cout<<endl;
