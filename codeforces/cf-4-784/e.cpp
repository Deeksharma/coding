...
void solve() {
	...
	int present_at1_but_not_present_at2[20][20], not_present_at1_but_present_at2[20][20];
	memset(present_at1_but_not_present_at2, 0, sizeof(present_at1_but_not_present_at2));
	memset(not_present_at1_but_present_at2, 0, sizeof(not_present_at1_but_present_at2));

	// for(int i=0;i<20;i++){
	// 	for(int j=0;j<20;j++){
	// 		cout<<present_at1_but_not_present_at2[i][j]<<" ";
	// 	}
	// 	cout<<endl;
	// }
	int ans = 0;
	for(int i=n-1;i>=0;i--){
		...
		int x=v[i][0]-'a';
		int y=v[i][1]-'a';
		ans += present_at1_but_not_present_at2[x][y]+ not_present_at1_but_present_at2[x][y];
		...
		for(char j='a';j<='k';j++){
			if((j-'a')!=y){
				present_at1_but_not_present_at2[x][j-'a']++;
			}
		}
		for(char j='a';j<='k';j++){
			if((j-'a')!=x){
				not_present_at1_but_present_at2[j-'a'][y]++;
			}
		}
	}
